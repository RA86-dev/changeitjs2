...
console.clear()
 ...
 function detectWeather(printbool,compilebool) {
  const kelvin = 273;
  ...
  function handleSuccess(position) {
    ...
    // Check if there is a custom Api key:
    if (apiKey == "PUT_YOUR_API_KEY") {
      console.error('Custom API Key Required. PUT_YOUR_API_KEY is default typed text.')
    }
    ...
    fetch(apiUrl)
      ...
      .then(data => {
        ...
        if (compilebool) {
          ...
          var datacomp = JSON.stringify(data);
          console.log('Compiled Weather Info: \n', datacomp)
          return datacomp
        }
        
        if (printbool === true) {
          alert('Current Weather Information:');
          alert('Temperature: ' + temperature);
          alert('Weather: ' + weather);
          alert('Location Detected: ' + location);
          alert('Pressure: ' + pressure);
          alert('Temperature (Max): ' + tempmax);
          alert('Temperature (Min): ' + tempmin);
          alert('Wind Degrees: ' + wind_degrees);
          alert('Wind Speed: ' + wind_speed);
